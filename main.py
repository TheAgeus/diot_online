...
def cargar_datos():
    ...
    for i in range(2, wsf.max_row):
        col_fisica.append({
            ...
            "compl"     :   wsf["F" + str(i)].value if wsf["F" + str(i)].value is not None else ""
        })
    for i in range(2, wsm.max_row):
        col_morall.append({
            ...
            "compl"     :   wsm["F" + str(i)].value if wsm["F" + str(i)].value is not None else ""
        })
    wb.close()
...
def mostrar_progreso_f(col_fisica):
    if len(col_fisica) == 0:
        ...
        return
    print(" +---------------+-----------------+-------+--------------------------------+-------------------+")
    print(" | RFC           | EMPRESA         | SUBID | ERROR                          | COMPLEMENTARIA    |")
    print(" +---------------+-----------------+-------+--------------------------------+-------------------+")
    for i in range(len(col_fisica)):
        ...
        compl = col_fisica[i]['compl'] if col_fisica[i]['compl'] != "" else "??" # Si o No
        print(f" | {col_fisica[i]['rfc']} | {col_fisica[i]['empresa'][:15].ljust(15)} | {subido} | {error} | {compl}                |")
        print(" +---------------+-----------------+-------+--------------------------------+-------------------+")
...
def mostrar_progreso_m(col_morall):
    ...
    for i in range(len(col_morall)):
        subido = str(col_morall[i]['subido']).ljust(5) if col_morall[i]['subido'] != "" else "No".ljust(5)
        error = col_morall[i]['error'][:30].ljust(30) if col_morall[i]['error'] != "" else "Aún no se ha realizado el proceso"[:30].ljust(30)
        compl = col_morall[i]['compl'] if col_morall[i]['compl'] != "" else "??"  # Si o No
        print(f" | {col_morall[i]['rfc']}  | {col_morall[i]['empresa'][:15].ljust(15)} | {subido} | {error} | {compl}                |")
        print(" +---------------+-----------------+-------+--------------------------------+-------------------+")
...
def proceso_fisicas(col_fisicas):
    for i in range(len(col_fisicas)):
        ...
        if acuse_file:
            ...
            col_fisicas[i]['compl'] = 'No'
            salvar_en_bitacora(col_fisicas[i], i, 'Fisica')
            continue

        d = webdriver.Edge()                       #Aqui es para validar que tipo de PAgina WEB FISICAS se ingresa
        ...
        if se_pico :
            d, es_complementaria = ingresa_per_para_continuar(d)    # Ingresar periodo y el ejercicio
            if not es_complementaria:
                ...
                if acuse_file:
                    ...
                    col_fisicas[i]['compl'] = 'No'
                    salvar_en_bitacora(col_fisicas[i], i, 'Fisica')
            else: # Es complementaria
                col_fisicas[i]['subido'] = '1'
                col_fisicas[i]['error'] = 'Ninguno'
                col_fisicas[i]['compl'] = 'Si'
                salvar_en_bitacora(col_fisicas[i], i, 'Fisica')
...
def proceso_morales(col_morales):
   for i in range(len(col_morales)):
        ...
        if acuse_file:
            ...
            col_morales[i]['comp'] = 'No'
            salvar_en_bitacora(col_morales[i], i, 'Moral')
            continue

        d = webdriver.Edge()              #Aqui es para validar que tipo de PAgina WEB MORALES se ingresa
