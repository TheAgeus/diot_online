...
def cargar_datos():
    ...
    for i in range(2, wsf.max_row):
        ...
        col_fisica.append({
            ...
            "complemen" :   wsf["H" + str(i)].value if wsf["H" + str(i)].value is not None else "" # Modificacion, no descargar complementaria. 24/05/2025

        })
    for i in range(2, wsm.max_row):
        if wsm["A" + str(i)].value == ".": break
        col_morall.append({
            ...
            "anio"      :   wsm["F" + str(i)].value if wsm["F" + str(i)].value is not None else "",
            "mes"       :   wsm["G" + str(i)].value if wsm["G" + str(i)].value is not None else "",
            "complemen" :   wsm["H" + str(i)].value if wsm["H" + str(i)].value is not None else "" # Modificacion, no descargar complementaria. 24/05/2025
        })
    wb.close()
...
def mostrar_progreso_f(col_fisica):
    if len(col_fisica) == 0:
        ...
        return
    print(" +---------------+-----------------+-------+--------------------------------+--------+-------+------------------+")
    print(" | RFC           | EMPRESA         | SUBID | ERROR                          | AÑO    | MES   | COMPLEMENTARIA   |")
    print(" +---------------+-----------------+-------+--------------------------------+--------+-------+------------------+")
    for i in range(len(col_fisica)):
        ...
        complementaria = col_fisica[i]['complemen'] # Si o No
        print(f" | {col_fisica[i]['rfc']} | {col_fisica[i]['empresa'][:15].ljust(15)} | {subido} | {error} | {anio}   | {str(mes).zfill(2)}    | {complementaria}               |")
        print(" +---------------+-----------------+-------+--------------------------------+--------+-------+------------------+")
...
def mostrar_progreso_m(col_morall):
    ...
    for i in range(len(col_morall)):
        ...
        anio = str(col_morall[i]['anio'])
        mes = col_morall[i]['mes']
        complementaria = col_morall[i]['complemen'] # Si o No
        print(f" | {col_morall[i]['rfc']}  | {col_morall[i]['empresa'][:15].ljust(15)} | {subido} | {error} | {anio}   | {str(mes).zfill(2)}    | {complementaria}               |")
        print(" +---------------+-----------------+-------+--------------------------------+--------+-------+------------------+")
...
def proceso_fisicas(col_fisicas):
    for i in range(len(col_fisicas)):
        ...
        if se_pico :
            d, es_complementaria = ingresa_per_para_continuar(d, año, mes)      # Ingresar periodo y el ejercicio. 24/05/2025 Modificacion, no descargar complementaria
            if not es_complementaria:                                           # Modificacion, no descargar complementaria. 24/05/2025
...
def proceso_morales(col_morales):
   for i in range(len(col_morales)):
        ...
        if se_pico :
            d, es_complementaria = ingresa_per_para_continuar(d, año, mes)  # Ingresar periodo y el ejercicio. 
            if not es_complementaria:                                       # Modificacion, no descargar complementaria. 24/05/2025
